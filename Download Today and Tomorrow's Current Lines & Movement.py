...
def line_movement_soup(soup, game_date,driver,game_half):
    ...
    for num in range(len(eventlines)):
        ...
        a_pit_name = A_pit_list[ngames]
        h_pit_name = H_pit_list[ngames]

        eventcode = eventlines[num]
        eventid = eventcode['id']
        book_id = eventid[[m.start() for m in re.finditer(r"-",eventid)][1]+1:]
        
        #book_id = eventlines[num]['id'][[m.start() for m in re.finditer(r"-",eventlines[num]['id'])][1]+1:]
...
def get_line_move_data(soup,game_date,game_half,book_name,a_pit_name,h_pit_name,line_change_list = []):
    ...
    df_line_moves = DataFrame(columns=('Date','Book','Team','Team SP','Opponent','Opponent SP','Full or Half Game?', 'Line Type','Time of Line Change','over.under','Line','Odds'))
...
def select_and_rename(df, text):
    ...
    if text[-2:] == 'ml':
        df = df[['key','day_game_nbr','time','team','pitcher','hand','opp_team',
                 'pinnacle','5dimes','heritage','bovada','betonline']]
    ## Change column names to make them unique
        df.columns = ['key','day_game_nbr',text+'_time','team','pitcher','hand','opp_team',
...
def main(driver, season, inputdate = str(date.today()).replace('-','')):
    ...
    write_df = write_df.merge(
                df_rl, how='left', on = ['key','day_game_nbr','team','pitcher','hand','opp_team'])
    write_df = write_df.merge(
                df_tot, how='left', on = ['key','day_game_nbr','team','pitcher','hand','opp_team'])
    write_df = write_df.merge(
                df_1h_ml, how='left', on = ['key','day_game_nbr','team','pitcher','hand','opp_team'])
    write_df = write_df.merge(
                df_1h_rl, how='left', on = ['key','day_game_nbr','team','pitcher','hand','opp_team'])
    write_df = write_df.merge(
                df_1h_tot, how='left', on = ['key','day_game_nbr','team','pitcher','hand','opp_team'])
    ## Get tomorrow's lines
    # if time.ml[:2] >= 12:
    ...
    ## Write to txt
    with open(os.getcwd()+'\\SBR_MLB_'+season+'_Closing_Lines_daily.txt', 'a') as f:
        write_df.to_csv(f, index=False, header = False)

def run_main(driver, season, gdate):
    bad_games = []
...
if __name__ == '__main__':
    ...
    f = open(os.getcwd()+'\\SBR_MLB_'+season+'_Line_Moves_daily.txt', 'a')
    f.write('Date,Book,Team,Team_SP,Opp,Opp_SP,Bet_Length,Bet_Type,Line_Move_Time,Over_Under,Line,Odds')
    f.write('\n')
    f.close()
